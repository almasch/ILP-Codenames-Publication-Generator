...
@task(name="combined",)
def combined_knowledge_tree(ctx):
    """
    Generates Aleph experiments with a combined knowledge tree.
    """
    relation = ["HAS_HYPERNYM", "HAS_COMPONENT_HOLONYM", "HAS_MEMBER_HOLONYM", "HAS_PORTION_HOLONYM", "IS_RELATED_TO"]

    #create the output folder
    sub_folder_path = f"{ctx.aleph_experiments_dir}/{ctx.exp_combined_dir}"
    ...
    for seed in read_seeds_from_file(ctx.seeds_file):
        ...
        copy_template_files(seed_folder, "combined")

        hyper = combined_hypernym_knowledge("noun", words, relation, ctx.neo4j_location, normal_knowledge_tree=False)
